...
const map<int, string>& Card::getMapPlace() const {
	return mapPlace;
}
...
const int Card::identify(string path, string& txt)
{
	...
	// 分析身份证号码得到信息
	// 获取省份信息
	string provinceCode = txt.substr(0, 2);
	string province = mapPlace[stoi(provinceCode)];

	// 获取出生日期信息
	string birthYear = txt.substr(6, 4);
	string birthMonth = txt.substr(10, 2);
	string birthDay = txt.substr(12, 2);

	// 获取性别信息
	char genderCode = txt[16];
	string gender = (genderCode - '0') % 2 == 0 ? "Female" : "Male";

	// 输出识别结果
	cout << "Identification Number: " << txt << endl;
